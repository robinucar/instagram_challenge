...
RSpec.describe User, type: :model do
  ...
  context("User can be created with vaild information") do
    ...
    it "User has a username" do
      expect(valid_user.username).to eq('test username')
    end
  ...
  context("User cannot be created with invalid information") do
    ...
    it 'will not accept a pre-existing email ' do
      User.create({username: 'username', name: 'name', password: 'password', email: 't@test.com'})
      expect(User.new({username: 'username', name: 'name', password: 'password', email: 't@test.com'}).valid?).to be_falsey
    end

    it 'is invalid if password is too short' do
      expect(User.new({username: 'username', name: 'name', password: 'pass', email:' e@mail.com'}).valid?).to be_falsey
    end
  end
end